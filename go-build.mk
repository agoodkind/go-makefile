...
DIST_DIR ?= dist
_GO_MK_DIST_BIN := $(DIST_DIR)/$(BINARY)

INSTALL_DIR ?= $(or $(XDG_BIN_HOME),$(HOME)/.local/bin)
_GO_MK_INSTALL_BIN := $(INSTALL_DIR)/$(BINARY)
...
# $(_GO_MK_DIST_BIN) or $(_GO_MK_INSTALL_BIN).
...
_GO_MK_DIST_OUTPUTS    := $(_GO_MK_DIST_BIN)
_GO_MK_INSTALL_OUTPUTS := $(_GO_MK_INSTALL_BIN)
else
...
_GO_MK_GIT_COMMIT  := $(shell git rev-parse --short HEAD 2>/dev/null || echo unknown)
_GO_MK_GIT_VERSION := $(shell git describe --tags --always --dirty 2>/dev/null || echo dev)
_GO_MK_GIT_DIRTY   := $(shell git diff --quiet 2>/dev/null && echo false || echo true)
_GO_MK_BUILD_TIME  := $(shell date -u +%Y-%m-%dT%H:%M:%SZ)
...
ifneq ($(strip $(VPKG)),)
GO_BUILD_LDFLAGS += \
	-X $(VPKG).Commit=$(_GO_MK_GIT_COMMIT) \
	-X $(VPKG).Version=$(_GO_MK_GIT_VERSION) \
	-X $(VPKG).Dirty=$(_GO_MK_GIT_DIRTY) \
	-X $(VPKG).BuildTime=$(_GO_MK_BUILD_TIME)
endif

ifneq ($(strip $(GKLOG_VPKG)),)
GO_BUILD_LDFLAGS += \
	-X $(GKLOG_VPKG).Version=$(_GO_MK_GIT_VERSION) \
	-X $(GKLOG_VPKG).Commit=$(_GO_MK_GIT_COMMIT) \
	-X $(GKLOG_VPKG).Dirty=$(_GO_MK_GIT_DIRTY) \
	-X $(GKLOG_VPKG).BuildTime=$(_GO_MK_BUILD_TIME) \
	-X $(GKLOG_VPKG).BinHash=
endif

GO_BUILD_TAGS          ?=
_GO_MK_TAGS_FLAG     := $(if $(strip $(GO_BUILD_TAGS)),-tags '$(GO_BUILD_TAGS)',)
_GO_MK_LDFLAGS_FLAG  := $(if $(strip $(GO_BUILD_LDFLAGS)),-ldflags '$(GO_BUILD_LDFLAGS)',)
...
# the same vars.
GO_BUILD_FLAGS := $(_GO_MK_TAGS_FLAG) $(_GO_MK_LDFLAGS_FLAG) $(GO_BUILD_EXTRA_FLAGS)
...
# after a commit, a tag, or a move between a clean and a dirty tree. _GO_MK_BUILD_TIME
...
_GO_MK_SETTINGS := \
	...
	commit=$(_GO_MK_GIT_COMMIT) version=$(_GO_MK_GIT_VERSION) dirty=$(_GO_MK_GIT_DIRTY) \
...
# sources rather than on each other: chaining $(_GO_MK_INSTALL_BIN) to $(_GO_MK_DIST_BIN)
...
version-info:
	...
	@echo "commit:      $(_GO_MK_GIT_COMMIT)"
	@echo "version:     $(_GO_MK_GIT_VERSION)"
	@echo "dirty:       $(_GO_MK_GIT_DIRTY)"
	@echo "build_time:  $(_GO_MK_BUILD_TIME)"

...
The CI quality jobs restore the same cache and never save it. Each quality job builds the key for its own platform on the same runner image as the compile job. The keys match when the compiler and the toolchain match. The quality gates read the dependency stamps in the restored prefix and skip the C build.

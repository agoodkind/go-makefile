...
The CI quality jobs restore the same cache and never save it. Each quality job computes the cache key for its own platform on the same runner image as the compile job. On a key match, the quality gates find the dependency stamps in the restored prefix and skip the C build. The key matches when the compiler and the toolchain match the compile job's.
